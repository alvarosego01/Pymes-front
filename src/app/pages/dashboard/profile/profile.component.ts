...
export class ProfileComponent implements OnInit {
  ...
  constructor(
    ...
    var dpPrev = this.GlobalConfigService.departamentos.find(
      (d) => d.departamento === this.department
    );
    ...
    for (
      var i = 0;
      i < this._usersService.usuario._naturalEconomicActivity.length;
      ++i
    ) {
      this.estadosActividad.push(
        this._usersService.usuario._naturalEconomicActivity[i].typeActivity
      );
      ...
      switch (
        ...
        case "Producción":
          ...
          this.ProduccionActivity = this._usersService.usuario._naturalEconomicActivity[
            i
          ].details;
          break;
        case "Comercialización":
          this.ComercializacionActivity = this._usersService.usuario._naturalEconomicActivity[
            i
          ].details;
          break;
        case "Servicios":
          this.ServiciosActivity = this._usersService.usuario._naturalEconomicActivity[
            i
          ].details;
          break;
        case "Otro":
          this.OtroActivity = this._usersService.usuario._naturalEconomicActivity[
            i
          ].details;
          break;
        default:
  ...
  subirImagen(type = null, file = null) {
    ...
    this._usersService
      .changeFileUser(
        ...
        type,
        file,
        this._usersService.usuario._id
      )
      .then((resp) => {
        ...
        // let r_id = resp._id;
        // let rUser = resp.User;
        this._usersService.guardarStorage(
          this._usersService.usuario._id,
          this._usersService.token,
          resp["User"]
        );
  ...
  userModifyAccount(forma: NgForm, type) {
    if (this._usersService.usuario.role == "ADMIN_ROLE") {
      ...
      this.GlobalConfigService.spinner = true;
      //
      this._usersService.updateUserPUT(usuario, null, "natural").subscribe(
        (resp) => {
          //
          this.GlobalConfigService.spinner = false;
    ...
    if (this._usersService.usuario.role == "EMPRESA_ROLE") {
      ...
      this.GlobalConfigService.spinner = true;
      //
      this._usersService.updateUserPUT(null, usuario, "company").subscribe(
        (resp) => {
          //
          this.GlobalConfigService.spinner = false;
  ...
  newEmailRequest(forma: NgForm) {
    ...
    this._notifyService.swalNormal
      .fire({
        ...
        text:
          "Debes confirmarlo accediendo a la bandeja bandeja de entrada de tu nuevo email",
        icon: "info",
  ...
  newEmail(email) {
    ...
    this._notifyService.sendNotifyEmailPOST(l, token).subscribe(
      (resp: any) => {
      ...
      (err) => {
        ...
        this._notifyService.Toast.fire({
          title:
            err.error.message ||
            "La solicitud no pudo ser enviada, intente más tarde",
          icon: "error",
        });
        this.GlobalConfigService.spinner = false;
      }
  ...
  userModifyPymeData(forma: NgForm) {
    ...
    this.GlobalConfigService.spinner = true;
    //
    this._usersService.updateCompanyDataPUT(usuario).subscribe(
      (resp) => {
        //
        this.GlobalConfigService.spinner = false;
  ...
  recoveryPassword() {
    ...
    let n = new _NotifyModel("nAccountPasswordChangeRequest", null, id);

    //////// console.log('la maldicion que se manda', n);
    this._notifyService.sendNotifyEmailPOST(n).subscribe(
      (resp: any) => {
        //// console.log('email enviado', resp);
