...
export class ProfileViewComponent implements OnInit {
  ...
  constructor(
    ...
    if (
      ...
      this._usersService.roleName == "Administrador"
    ) {
      this.canEdit = true;
    }
  }
  ...
  ngOnInit() {
    this.activatedRoute.params.subscribe(async (params) => {
      ...
      await this.setNewVisit(id).then(
        (r) => {
          //// console.log('verga', r);
        },
        (err) => {}
  ...
  setNewVisit(id) {
    return new Promise((resolve, reject) => {
      ...
      this._usersService.setNewVisitPUT(id).subscribe((resp) => {
  ...
  setUserInformation() {
    ...
    for (var i = 0; i < this.usuarioInfo._naturalEconomicActivity.length; ++i) {
      this.estadosActividad.push(
        this.usuarioInfo._naturalEconomicActivity[i].typeActivity
      );
      ...
      switch (this.usuarioInfo._naturalEconomicActivity[i].typeActivity) {
        case "Producción":
          ...
          this.ProduccionActivity = this.usuarioInfo._naturalEconomicActivity[
            i
          ].details;
          break;
        case "Comercialización":
          this.ComercializacionActivity = this.usuarioInfo._naturalEconomicActivity[
            i
          ].details;
          break;
        case "Servicios":
          this.ServiciosActivity = this.usuarioInfo._naturalEconomicActivity[
            i
          ].details;
          break;
        case "Otro":
          this.OtroActivity = this.usuarioInfo._naturalEconomicActivity[
            i
          ].details;
          break;
        default:
    ...
    this.GlobalConfigService.setTitle(
      `Perfil de usuario - ${this.usuarioInfo.name} ${this.usuarioInfo.surname}`
    );
  }

  getUserProfile(idUser: string) {
    this.GlobalConfigService.spinner = true;
    this._usersService.getUserProfileGET(idUser).subscribe(
      (resp) => {
        // ////// ////// console.log(resp);
  ...
  openEdit() {
    ...
    // console.log('manda aabrir,', this.editOpen);
    // console.log('manda user,', this.usuarioInfo);
